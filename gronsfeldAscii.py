...
def gronsfeld_cypher(text, key):
  ...
  for i in range(0,len(text_list)): #iterate as many times as elements in text_list
    ...
    current_letter_ascii = ord(text_list[i]) #ascii for the current letter in text_list

    current_key = key_list[key_index] #number for the found key index

    resulting_letter_pos = current_letter_ascii - 64 + current_key
    
    if(resulting_letter_pos > 26): #
      resulting_letter_pos = resulting_letter_pos - 26

    resulting_letter = alphabet[resulting_letter_pos-1]

    encrypted_list.append(resulting_letter)
  print(encrypted_list)

gronsfeld_cypher("ABCDEFGHIJ$^KLMNOPQRSTUVWXYZ","9")
gronsfeld_cypher("ABCDEFGHIJKLMNOPQRSTUVWXYZ", "1gt")
gronsfeld_cypher("ABCDEFGHIJKLfgrgtNOPQRSTUVWXYZ","0")
gronsfeld_cypher("AXYZ","54444")
